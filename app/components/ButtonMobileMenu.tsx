...
export default function ButtonMobileMenu({ planet }: { planet: string; mode: string }) {
  const menu = '/menu';
  return (
    <Link className={!planet ? 'pointer-events-none' : ''} href={menu}>
      <button
        title="Menu"
        type="button"
        className={`relative flex h-[17px] w-[24px] items-center justify-center md:hidden ${planet ? 'fill-white' : 'fill-[#979797]'}`}
      >
